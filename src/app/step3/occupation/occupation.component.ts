...
export class OccupationComponent implements OnInit {
  ...
  async occupstore(event:any) {
    ...
    console.log(this.occup.names);
    const paramspJ = {body: {userid: user.attributes.sub, occup: this.occup.names}};
    API.put("datingapitest4", "/userdbapioccup", paramspJ).then(responseJ => {{console.log("successJ");}}).catch(error => {console.log(error.responseJ);});
  }
